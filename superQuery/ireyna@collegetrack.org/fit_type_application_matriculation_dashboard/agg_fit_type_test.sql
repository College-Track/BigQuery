...
SELECT
    accept.contact_id,
    accept.Full_Name__c,
    IF(accept.college_id = '0014600000plKMXAA2',"Global Citizen Year",accnt.name) AS school_name_accepted,
    accept.College_Fit_Type_Applied__c AS fit_type_applied_accepted,
    CASE
        WHEN accept.admission_status__c IN ("Accepted", "Accepted and Enrolled", "Accepted and Deferred") THEN "Accepted"
        WHEN accept.admission_status__c = "Denied" THEN "Denied"
        WHEN accept.admission_status__c = "Coniditional" THEN "Conditional"
        WHEN accept.admission_status__c = "Undecided" THEN "Undecided"
        WHEN accept.admission_status__c = "Withdrew Application" THEN "Withdrew Application"
        WHEN accept.admission_status__c = "Wait-listed" THEN "Waitlisted"
        ELSE "No data"
    END AS acceptance_group_accepted
    
FROM `data-studio-260217.fit_type_pipeline.filtered_college_application` AS accept
LEFT JOIN `data-warehouse-289815.salesforce_raw.Account` AS accnt
        ON accept.college_id = accnt.id
        
WHERE accept.Indicator_Completed_CT_HS_Program__c = TRUE
AND accept.admission_status__c IN ("Accepted", "Accepted and Enrolled", "Accepted and Deferred")
AND accept.High_School_Class >= 2017 #c/o 2017 and above have most completed Fit Type (applied) and Fit Type (enrolled) 
AND accept.Application_status__c = "Applied"
),
