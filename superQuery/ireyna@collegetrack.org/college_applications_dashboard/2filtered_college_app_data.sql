...
LEFT JOIN `data-warehouse-289815.salesforce_raw.College_Application__c` AS CA 
        ON C.Contact_Id = CA.Student__c
...
FROM `data-warehouse-289815.salesforce_raw.College_Aspiration__c` AS CAP
...
LEFT JOIN college_aspirations AS asp
    ON app.contact_id = asp.Student__c
)
SELECT *
FROM final_join