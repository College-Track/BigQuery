...
SELECT 
    DISTINCT
    A.*,
    B.* EXCEPT (site_short,Region),
    C.* EXCEPT (Region)
FROM hr_tenure AS A                    
LEFT JOIN financial_sustainability AS B     ON A.Site = B.site_short AND A.Account = B.Account  
...
SELECT 
    * EXCEPT (Account, Site),
    mapSite(Account) AS site,
    mapRegion(Account) AS Region,
