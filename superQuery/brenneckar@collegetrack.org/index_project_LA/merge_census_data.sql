...
-- -(avg_percent_housing_units_renter_occupied - avg(avg_percent_housing_units_renter_occupied) over()) / stddev(avg_percent_housing_units_renter_occupied) over() AS housing_units_renter_occupied, --negative
-- (avg_income_per_capita - avg(avg_income_per_capita) over()) / stddev(avg_income_per_capita) over() AS income_per_capita,--positive
-- (avg_median_income - avg(avg_median_income) over()) / stddev(avg_median_income) over() AS median_income,--positive
-- -(avg_percent_not_us_citizen_pop - avg(avg_percent_not_us_citizen_pop) over()) / stddev(avg_percent_not_us_citizen_pop) over() AS percent_not_us_citizen_pop,--negative
-- -(avg_percent_income_spent_on_rent - avg(avg_percent_income_spent_on_rent) over()) / stddev(avg_percent_income_spent_on_rent) over() AS percent_income_spent_on_rent,--negative
-- -(avg_percent_unemployed_pop - avg(avg_percent_unemployed_pop) over()) / stddev(avg_percent_unemployed_pop) over() AS percent_unemployed_pop,--negative
-- -(avg_percent_vacant_housing_units - avg(avg_percent_vacant_housing_units) over()) / stddev(avg_percent_vacant_housing_units) over() AS percent_vacant_housing_units,--negative
