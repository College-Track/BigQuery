...
--cohort based, trending is students in year 2. final calc is done in fall of year 3
    
--numerator for trending shows which of the 2-year startes in year 2 have already enrolled in a 4-year. Will need to be reworked for final
    CASE
        WHEN
...
--denom for trending, in year 2 started at 2year or lower. Will need to be reworked for final
    CASE
        WHEN
        (indicator_completed_ct_hs_program_c = true
        AND grade_c = 'Year 2'
...
--Will probably want to re-work for final calc
    CASE
        WHEN
        (indicator_completed_ct_hs_program_c = true
        AND college_track_status_c = '15A'
...
--indicator to flag which students were enrolled in any college last fall. used to created denomiator later
    MAX(CASE
        WHEN
...
--counting the # of ATs for each student
...
--for those same records, counting # of ATs for each student in which they met term to term persistence definition
    ...
    FROM `data-warehouse-289815.salesforce_clean.contact_at_template`
--Start date for PAT must be after today to be included. To exclude future PATs upon creation, until they become current AT
...
(
  ...
  -- if # terms vs. # of terms meeting persistence defintion student will be in numerator
    MAX(
    CASE
