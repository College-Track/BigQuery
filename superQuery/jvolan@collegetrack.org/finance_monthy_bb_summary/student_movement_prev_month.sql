...
AS( 
    ...
    AND extract(Month FROM Start_Date_c) = extract(month from date_sub(current_date,interval 1 Month))
    AND start_date_c + 365 >= CURRENT_DATE()
)   
    SELECT 
    status_history.*, 
    ct_site.student, 
    ...
    FROM status_history 
    LEFT JOIN ct_site ON ct_site.contact_id = contact_c WHERE ct_site.site_short NOT IN ('Arlen')