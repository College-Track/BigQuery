...
--we introduce a dummy row for each student that has SLA data within the join_data file just created above
--this is done as a step just prior to doing a running sum partition so we can
   -- 1. for each student - we create a dummy row that has a 1 in the dummy_data_row column, so we can identify dummy row vs. real SLA data
   -- 2. we edit the date on the dummy row to be a week prior to SLA date (week is arbitrary, but we need dummy row to always be ordered before real SLA data
   -- 3. The only other useful data on dummy row is the MAX(cs_1600_cap) AS hours_dollar_amount , which is the main purpose of this row. 
         --This sets the student's current total service + ct advised earnings (ie cs_1600_cap) as the initial hours_dollar_amount, so that when we add in the SLA data in the running sum partition, it will add all new hours to this baseline amount
...
-- now that we have the data set from the above Union All, we have a dummy row and SLA row for each student and they are then ordered by contact_id & created date
-- we then do a running sum of the hours_dollar_amount field to give us an idea of where student stands in relation to 1600 cap with new hours added in
...
--this the compares total hours/eligble bb dollars from newly approved SLAs vs student's 1600 cap to determine how much, if any, new service BB earnings the student will get
bb_earn_calc AS
(
    SELECT
    *, 
